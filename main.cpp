...
void  main_func(Context& context)
{
    ...
    std::vector<char> v;
    v.resize(context.block_size);
    context.array=v.data();
    ...
    //Функция отсеивающая все уникальные файлы в мультисете и рассчитывающая следующий блок ХЭШа
        auto func2=[](std::multiset<FileInfo>& m1, Context& context)
        {
            if (m1.size()<2) return;
            std::multiset<FileInfo> m2;
            bool divider=false;
            for (auto it=m1.begin(); it!=--(m1.end()); it++)
            {
                ...
                if (*it==*it2)
                {
                    ...
                    {
                        FileInfo t1(*it);
                        t1.calculate(context.block_size, context.array, context.hash_alg);
                        m2.insert(t1);
                    }
                    FileInfo t2(*it2);
                    t2.calculate(context.block_size, context.array, context.hash_alg);
                    m2.insert(t2);
                    divider=true;

                }
                else if (divider) divider=false;
            }
    ...
    for(auto x: filelist)
    {
        ...
        if (not_exclude(patch1.generic_string()))//тут должна быть проверка на принадлежность файла к исключенным каталогам
        {
            FileInfo myfile(patch1.generic_string());
            myfile.calculate(context.block_size, context.array, context.hash_alg);
    ...
    //На этом этапе имеем мультисет с начальным набором файлов
    //Будем считать в них ХЭШ по одному блоку до тех пор пока не дойдем до конца самого длинного из них
    //одновременно убирая уникальные файлы из рассмотрения

    bool stop=false;
       while (!stop)
       {
           func2(multiset, context);
           stop=true;
           for (auto x: multiset)
           {
               auto temp=x.get_stop();
               stop=(!temp)? false : stop;
           }
       }
